...
| Type | Emoji | Meaning |
|------|-------|---------|
| `PLANNING` | [PLAN] | Agent is planning its approach |
| `SEARCHING` | [SEARCH] | Agent is querying CloudWatch, X-Ray, logs, IAM, etc. |
| `ANALYSIS` | [ANALYSIS] | Agent is analyzing collected data |
| `FINDING` | [FINDING] | Key discovery -- surface this prominently |
| `ACTION` | [ACTION] | Agent is performing a read-only action |
| `SUMMARY` | [SUMMARY] | Investigation summary with root cause |
