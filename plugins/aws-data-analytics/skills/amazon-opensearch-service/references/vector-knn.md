...
**On Serverless NextGen Vector**, **FAISS HNSW IS supported** -- the customer doesn't choose the engine, the system selects FAISS HNSW under the hood (Lucene HNSW, IVF, and PQ cannot be pinned on NextGen). Custom doc IDs supported. 32x compression default. 10s refresh interval.
...
```
bytes_per_vector ~ 1.1 x (4 x dim + 8 x m)
total_memory ~ bytes_per_vector x num_vectors x (1 + replicas)
```
...
**AWS budget formula:** `memory_available = (RAM - jvm_size) x circuit_breaker_limit`

- `jvm_size = min(0.5 x RAM, 32 GiB)`
- `circuit_breaker_limit = 0.5` (default)

**Example:** `r7g.4xlarge.search` = 128 GiB RAM, JVM = 32 GiB, available for k-NN graphs ~ `(128 - 32) x 0.5 = 48 GiB`.
...
Memory ratios (stable): fp32->fp16 = 2x, fp32->int8 = 4x, fp32->int4 = 8x, fp32->binary = 32x.
...
Formula: `rankScore(d) = sum 1/(k + rank_i)` where `k = rank_constant` (default 60).
...
- **GPU-accelerated index build**: up to **9.3x faster, 3.75x cost reduction**
- **Derived-source vectors**: 3x storage reduction, 30x cold-start improvement
- **Concurrent segment search default-on for k-NN**: 2.5x boost
- **Star-tree indexing**: aggregations up to 100x faster
...
7. **`uw.medium` cannot host k-NN** -- RAM headroom insufficient. Use `uw.large` and size graphs <= `circuit_breaker_limit x 61 GiB` per instance.
