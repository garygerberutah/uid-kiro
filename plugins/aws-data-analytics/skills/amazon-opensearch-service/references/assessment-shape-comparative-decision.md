...
When the prompt names **3+ hard constraints** (e.g., zero downtime, zero data loss, no third-party tooling, EU residency, fixed budget, fixed deadline) and asks "how do you reconcile these?" -- the user is asking for a **feasibility ruling**, NOT a SKU pick. Before the Pick (Section 3.1), insert a **Constraint feasibility** block:
...
Then in Section 3.1 Pick, recommend the **relaxation**, not just the SKU:
...
In the Section 3.2 comparison table, add a **Relaxation** column showing what each option costs you (which constraint it forces to bend). Decision driver (Section 3.3) names the conflict explicitly: "this option wins because it minimizes the relaxation needed on the load-bearing constraint."
...
- [NO] Claiming a single path "simultaneously satisfies" all 3+ constraints when it cannot -- the rubric will fail you for not surfacing the conflict.
- [NO] Picking a path that requires dual-write under a "no third-party tooling" constraint without the reconciliation rule above.
- [NO] Treating the prompt as "which AWS SKU?" instead of "which constraint do we relax?" -- these prompts are about **trade-offs**, not about Managed vs Serverless.
