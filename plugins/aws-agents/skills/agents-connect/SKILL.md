...
[WARNING] **Security note:** `--api-key` appears in shell history. Two safer options:
...
[WARNING] **Security note:** `--api-key` and `--client-secret` appear in shell history. Run the command without those flags to get an interactive prompt -- the value goes straight into the credential provider without touching your shell history.
