...
|---|---|---|
| Agent invocation | [YES] via curl on localhost:8080 | [YES] via `invoke_agent_runtime` or HTTPS |
| Framework model calls | [YES] if Bedrock creds are available | [YES] |
| Python/JS function tools (framework-native) | [YES] | [YES] |
| Credentials (`@requires_api_key`, `@requires_access_token`) | [YES] from `agentcore/.env.local` | [YES] from Secrets Manager |
| Memory | [NO] env var not set locally | [YES] `MEMORY_<NAME>_ID` injected |
| Gateway | [NO] env var not set locally | [YES] `AGENTCORE_GATEWAY_<NAME>_URL` injected |
| Cedar policy evaluation | [NO] policies only enforced at gateway | [YES] |
| Traces (X-Ray) | [YES] `agentcore dev` emits OTEL to CloudWatch by default; disable with `--no-traces` | [YES] auto-enabled |
| CloudWatch logs | [YES] via ADOT / OTEL wiring (same path as traces) | [YES] if using `logging` module + OTEL |
| **Evaluator *definition*** (`agentcore add evaluator`, writing the instructions/code) | [YES] -- writes to `agentcore.json`; custom code is unit-testable locally | [YES] |
| **`agentcore run eval`** (on-demand eval over traces) | [YES] -- operates on CloudWatch spans; local-dev spans land there if OTEL is on (default) | [YES] |
| **`Evaluate` API with hand-constructed spans** (boto3) | [YES] -- no runtime needed at all; submit `SessionSpans` directly | [YES] |
| **Dataset runner** (`OnDemandEvaluationDatasetRunner`) | [NO] invokes an AgentCore Runtime agent in its pipeline | [YES] |
| **Online eval monitoring** (`agentcore add online-eval`) | [NO] ingests traces continuously from deployed runtime | [YES] |
| Observability dashboards | [YES] once Transaction Search is on and local spans are flowing | [YES] in CloudWatch console |
| VPC networking | [NO] local always has internet | [YES] subject to `networkMode: VPC` |
| Inbound auth (AWS_IAM, CUSTOM_JWT) | [NO] no auth required locally | [YES] enforced on every request |
