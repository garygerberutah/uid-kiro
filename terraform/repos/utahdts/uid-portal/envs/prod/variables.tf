# Variables for the prod environment. Generated from modules/api_environment/variables.tf;
...
variable "vpc_ipv4_cidr" {
  description = "Owner-supplied canonical primary IPv4 CIDR of the existing production VPC."
  type        = string
  ...
  validation {
    ...
    error_message = "vpc_ipv4_cidr must be the canonical primary IPv4 CIDR confirmed by the production network owner; REPLACE_ME deliberately blocks planning."
  }
}

variable "private_subnet_ids" {
  description = "Existing production private subnets supplied by the State network owner."
  type        = list(string)
  ...
  validation {
    ...
    error_message = "private_subnet_ids must contain at least two distinct existing production subnet ids confirmed by the State network owner."
  }
}
