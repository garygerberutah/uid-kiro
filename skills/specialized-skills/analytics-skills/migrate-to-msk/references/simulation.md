...
The fleet is sized to ~1.5x the cluster's **maximum** ingress quota (its throttle
...
(MSK throttles read/write traffic beyond it). Cluster totals = per-broker x broker
...
count scales: `fleet_producer_count = max(2, ceil(cluster_max_ingress x 1.5 /
...
cluster (the values below are for the 12x express.m7g.4xlarge default).
