...
| Code | Severity | When |
|---|---|---|
| `VERSION_SUPPORTED` | INFO | `kafka.version` in {3.6, 3.8, 3.9} |
...
| Config | Bound |
|---|---|
| `log.cleaner.max.compaction.lag.ms` | [1 day = 86_400_000 ms, +infinity] |
| `max.compaction.lag.ms` (topic) | [1 day = 86_400_000 ms, +infinity] |
...
| `CONNECTIONS_OVER_IAM_LIMIT` | ADVISORY | `security.authentication` in {`SASL_IAM`, `SASL_OAUTHBEARER`} AND `peak_connections_per_broker` > 3_000 |
| `PARTITION_THROUGHPUT_OVER_LIMIT` | ADVISORY | average per-partition throughput (`peak_in x num_brokers / total_partitions`) > 15 MB/s |
