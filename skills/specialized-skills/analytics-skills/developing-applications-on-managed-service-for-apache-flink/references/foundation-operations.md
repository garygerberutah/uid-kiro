...
|------|-------|---------------------|
| AWS CLI service / SDK client | `kinesisanalyticsv2` | [NO] `flink`, `msf`, `kinesisanalytics` (v1, deprecated SQL apps only) |
| Service Quotas service-code | `kinesisanalytics` (no `v2`) | [NO] `kinesisanalyticsv2`, `flink`, `msf` |
| IAM action prefix | `kinesisanalytics:` (no `v2`) | [NO] `kinesisanalyticsv2:` |
| CloudWatch namespace | `AWS/KinesisAnalytics` | [NO] `AWS/Flink`, `AWS/ManagedFlink` |
| Trust policy principal | `kinesisanalytics.amazonaws.com` | [NO] `kinesisanalyticsv2.amazonaws.com` |
...
**RDS/Aurora sink anti-pattern:** Per-record JDBC writes are 10-100x slower than batched writes. Use the JDBC sink with batching enabled. RDS connection pool limits (typically 5,000) cap effective parallelism.
