...
|--------|-----------|---------------------|-------------|
| Parallelism 4 -> 8, PPK=1 | 4 -> 8 KPU | 4 GB (unchanged) | 2x |
| Parallelism 4 -> 8, PPK=2 | 2 -> 4 KPU | 2 GB (unchanged) | 2x |
| PPK 2 -> 1, Parallelism=8 | 4 -> 8 KPU | 2 -> 4 GB | 2x |
| PPK 1 -> 2, Parallelism=8 | 8 -> 4 KPU | 4 -> 2 GB | 0.5x |
| Parallelism 8 -> 4, PPK=1 | 8 -> 4 KPU | 4 GB (unchanged) | 0.5x |
...
- [NO] Cannot scale during a transitional state (STARTING, UPDATING, STOPPING, AUTOSCALING)
- [NO] Cannot scale during another in-flight update -- sequence operations
- [WARNING] Every scaling change triggers a restart with 10-30s downtime (varies with state size and snapshot recency)
- [WARNING] Take a snapshot before scaling for rollback safety
- [WARNING] Scaling beyond 64 KPU requires a Service Quotas increase (default limit)
- [WARNING] Setting Parallelism > source partition/shard count creates idle subtasks -- wasted KPUs
