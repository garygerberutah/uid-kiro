...
| `IOException: Insufficient number of network buffers` | Buffer pool exhausted | parallelism x shuffle-edge fan-out and `taskmanager.network.memory.*` config |
...
| Data skew | Subtask `read-records` variance > 5x across the same vertex | `rebalance()` instead of `keyBy()` on hot keys; pre-split |
