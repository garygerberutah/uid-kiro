...
4. If the max value is more than 3x the median, the operator has significant skew

A subtask processing 10x more records than its peers will become the throughput bottleneck for the entire operator, regardless of how many other subtasks are idle.
...
4. **Key cardinality vs parallelism**: Ensure key cardinality is at least 10x the operator parallelism for reasonable distribution.
