...
- Migration Assistant for Amazon OpenSearch Service **does** support Solr backfill (and Live Traffic Migration). Do NOT tell a customer the service is Elasticsearch-only. For target restrictions and source/target eligibility, see Section "Source / target rules" below.
...
5. MIGRATION PATH -> for each component the customer needs (Historical Data Migration, Live Traffic Migration, Application Code Rewrite -- not all are required), pick a primary strategy from the per-component tables in Section "Components of a migration". Skip components that don't apply to this workload.
...
For **Business Stakeholder** persona, run the six-question intake first (see Section Business Stakeholder intake).
...
The Severity x Lane combination determines whether the row deducts from the Compatibility readiness weight (see [`readiness-rubric.md`](readiness-rubric.md) -- only `risk-blocker`-lane rows deduct).
...
| Deprecated/removed Solr field types (Trie*, etc.) | HIGH | migration-specific | For the full Solr 7/8/9 deprecation list, see assessment-shape-schema-conversion.md Section Section D -- Gap register. |
...
| Removed Solr handlers/writers (DIH, Velocity, XSLT, etc.) | HIGH | risk-blocker | For the full Solr 7/8/9 deprecation list, see assessment-shape-schema-conversion.md Section Section D -- Gap register. |
...
- Bursty traffic (10x+ swings) or zero-ops preference
...
- Redundancy ON: minimum 1 indexing OCU (0.5 x 2) + 1 search OCU (0.5 x 2) -- billed even idle
- Redundancy OFF: minimum 0.5 OCU x 2 for first collection
...
- Mixed keyword + vector (Classic) -> [WARNING] **Vector Search collections cannot share OCUs with Search/TimeSeries collections** -- doubles idle floor
...
For each of the three components, decide whether it applies and which strategy fits. Skip components that don't apply. See Section "Components of a migration" above for the strategy menu under each.
...
Once you've decided which components apply, pick the primary strategy under each from the per-component tables in Section "Components of a migration". Common combinations:
...
This is a fast lookup over the strategies in Section "Components of a migration -> Historical Data Migration". Pick by source profile.
...
- **Match-source rule** (when source sizing is provided): stay close to source RAM (8-16 GB per data node) unless workload signals (peak QPS, retention, page-cache headroom) justify uplift. Recommending 4x source RAM without explicit signal-based justification is a sizing miss.
- **Default starting point** (no source sizing provided): `3 x r7g.large.search` (8 GB heap each) + `3 x m7g.large.search` cluster managers + `gp3 200 GiB`. Multi-AZ.
- **Storage formula:** `source_data x (1 + replicas) x 1.45`.
- **Shard size:** 10-30 GiB for search, 30-50 GiB for write-heavy/logs.
- **Primary shards:** `(source + growth) x 1.1 / desired_shard_size`, rounded up to multiple of data-node count.
- **Cluster manager and per-node shard caps:** see [sizing.md Section Topology defaults](sizing.md). Source of truth: [bp.html#bp-sharding](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/bp.html#bp-sharding).
...
|---|---|---|
| <100 GB | 3 x m7g.large.search | Match-source for tiny |
| 100-500 GB | 6 x r7g.large.search | Standard mid-tier |
| >500 GB | 9 x r7g.2xlarge.search | Headroom for growth |
```
...
- **GREEN >= 80** -- proceed; surface top items to flag in Section 7 (split across Migration specifics and Risks/blockers)
