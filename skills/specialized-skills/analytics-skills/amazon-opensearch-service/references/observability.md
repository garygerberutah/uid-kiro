...
The summary version is in `SKILL.md` (Section Logs & observability). This file owns the deep playbooks: ISM lifecycle, Splunk/Datadog migration, Trace Analytics, alerting, cost optimization at scale.
...
- **Per-node shard cap (current values)**: see [sizing.md Section Topology defaults](sizing.md).
...
For log-analytics workloads, default to OR1 (write-heavy log profile) with UltraWarm tiering for >7-day retention. Full instance family list: [sizing.md Section Instance family selection](sizing.md). Source of truth: [supported-instance-types.html](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/supported-instance-types.html).
...
For non-critical observability indexes, `async` typically gives 2-5x ingest throughput improvement.
