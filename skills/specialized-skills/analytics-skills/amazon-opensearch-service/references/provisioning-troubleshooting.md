...
shards_per_node_cap    = 1000 x (heap_GiB / 16)                (OS >= 2.17 rule, capped at 4000)
                       = 25 x heap_GiB                           (legacy "safe target")
```

Worked example for the typical case (3 x `r7g.2xlarge.search` data nodes):

- Per `r7g.2xlarge.search`: 64 GiB RAM -> 32 GiB JVM heap (50% rule, 32 GiB cap)
- Per-node shard cap (OS >= 2.17): `32 / 16 x 1000 = 2000 shards/node` (hard ceiling)
- Per-node "safe" target: `25 x 32 = 800 shards/node`
