...
| Source cluster spec they want mapped | "we run 6 x `r5.2xlarge` today" |
| No `schema.xml`, no ES mapping, no "translate this query", no traffic-and-readiness mix | -- |
| Vector-search collection sizing without ingestion-pipeline questions | "50M x 768 vectors" |
...
> *Run **3 x `r7g.large.search`** data nodes + **3 x `m7g.large.search`** dedicated cluster managers across **3 AZs**, **1 replica**, EBS gp3 sized to **`<storage_number> GiB per data node`**.*
...
```
min_storage = source x (1 + replicas) x 1.45
            = 80 GiB x (1 + 1) x 1.45
...
- 6 x `r5.2xlarge` self-managed -> recommend 6 x `r7g.2xlarge.search` (Graviton equivalent), not "3 x `r7g.large` is our default."
- 4 x `m5.xlarge` self-managed -> recommend 4 x `m7g.xlarge.search`.
...
Only fall back to "3 x `r7g.large.search`" greenfield baseline when the source size is **<100 GB AND** the user provided no source cluster.
...
|---|---|---|---|
| Small | <100 GiB | 3 x `r7g.large.search` data + 3 x `m7g.large.search` cluster manager, 1 replica, gp3 100 GiB/node | Smallest prod-credible footprint |
| Medium | 100-500 GiB | 3 x `r7g.xlarge.search` data + 3 x `m7g.large.search` cluster manager, 1 replica, gp3 sized via formula | Most common SMB workload |
| Large | >500 GiB | 6+ x `r7g.2xlarge.search` data + 3 x `m7g.large.search` cluster manager, OR1 if write-heavy logs | Consider OR1/UltraWarm tiering |
...
**User asks:** *"We have 80 GB of product-search data on a self-managed Elasticsearch 7.10 cluster running 4 x `r5.xlarge`. What should we run on AOS?"*

**Response:**

*Detected shape: SIZING_ONLY -- baseline for 80 GiB product-search workload on Amazon OpenSearch Service, sized to match an existing 4 x `r5.xlarge` source.*

Run **4 x `r7g.xlarge.search`** data nodes + **3 x `m7g.large.search`** dedicated cluster managers across **3 AZs**, **1 replica**, EBS gp3 at **80 GiB per data node**.

- **Match-source:** the existing 4 x `r5.xlarge` already proves the working set fits that RAM-to-data ratio; mapping to Graviton (`r7g.xlarge.search`) preserves the profile and gains ~20% price/perf.
- **Master nodes:** required because data-node count >= 3; `m7g.large.search` x 3 covers up to 30 nodes / 15K shards on OS 2.17+ -- well above this footprint.
...
- **Shards:** for a search workload, target 10-30 GiB per shard. 80 GiB / 20 GiB -> 4 primary shards, aligned to data-node count. Final layout: 4 primary x 2 (1 replica) = 8 shards across 4 nodes.
...
- [ ] **Cluster managers explicitly addressed** -- present when >=3 data nodes or >=10 indexes; called out as `m7g.large.search` x 3 (or larger per the cluster-manager-sizing table in `sizing.md`).
