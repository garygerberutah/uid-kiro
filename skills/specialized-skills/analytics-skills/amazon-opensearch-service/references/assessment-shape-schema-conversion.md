...
> **Canonical reference.** This is the canonical Solr-7-and-9 field-type-and-deprecation reference for the skill. Other files (assessment-workflow Section X-Pack/Solr deprecation, assets/solr-gap-register, asset/report templates) link here for the exhaustive list.
...
- Standalone risks/blockers register beyond the gap-register deprecations in Section D -- SCHEMA_CONVERSION ships only the gap-register, not the FULL_ASSESSMENT Section 7 two-table split.
