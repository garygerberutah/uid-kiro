...
### Header (mandatory -- placed immediately after title, before Section 1)
...
Frame the migration around the **3 components** (see `references/assessment-workflow.md` Section "Components of a migration"):
...
```
storage_gb_per_node = (raw_gb x (1 + replicas) x (1 + overhead_0.15) x (1 + headroom_0.25)) / data_node_count
```

Required outputs:

- **Compute**: `<N>x <instance_class>` for data nodes (e.g., `6x r7g.2xlarge.search`) -- Graviton r7g/r8g default.
- **Cluster managers**: `3x <instance>` for >=6 data nodes (e.g., `3x m7g.large.search`).
...
4. **AWS Pricing Calculator** with the specific sizing inputs you derived. State *"plug instance class + count + storage from Section 5 into <https://calculator.aws>"* -- not generic.
...
| 3 | Plug sizing into Pricing Calculator | https://calculator.aws (use 6x r7g.2xlarge.search, 3x m7g.large.search, gp3 300 GB) |
...
>
> **Sizing.** `(600 x 2 x 1.15 x 1.25) / 6 = 287.5 GB/node` -> 6x `r7g.2xlarge.search` + 3x `m7g.large.search` cluster managers, gp3 300 GB/node. 18 primary shards x 1 replica ~ 33 GB/shard (in target band). JVM 32 GB heap -> shard cap 2,000/node (gotcha #4).
>
...
- [ ] **>=2 named gotchas** cited by number across Section 7 (Migration specifics + Risks/blockers; e.g., `#2`, `#11`).
- [ ] **>=3 citations** in section 9, each with URL + UTC timestamp + claim summary.
- [ ] Customer-specific trade-offs in Section 7 (not a generic recycled list).
- [ ] Items with a known fix routed to **Migration specifics**, not lumped under **Risks/blockers**.
- [ ] **Next Steps section (Section 8)** present with 5-7 concrete pointers -- each pointer is a skill name, a CLI command, an AWS docs URL, or `https://calculator.aws` with derived inputs. No generic "talk to your team" entries.
