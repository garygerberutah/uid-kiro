...
```
min_storage = source_data x (1 + replicas) x (1 + indexing_overhead) / (1 - linux_reserved) / (1 - aos_overhead)
```
...
**Simplified rule**: `min_storage ~ source_data x (1 + replicas) x 1.45`.
...
- `primary_shards = (source + room_to_grow) x 1.1 / desired_shard_size`, rounded up to multiple of data-node count
- `shards_per_node <= 25 x GiB_heap` -- e.g., 32 GiB heap = max 800 shards/node
...
- **FAISS HNSW + PQ**: trade recall for ~4-32x memory savings
- **FAISS HNSW + scalar quantization (16-bit)**: 2x memory savings, minimal recall loss
...
- Size so cumulative graph size of actively-searched shards <= `knn.memory.circuit_breaker.limit x 61 GiB` per `uw.large`
...
|---|---|---|---|
| Redundancy ON (production default) | 1 OCU (0.5 x 2) | 1 OCU (0.5 x 2) | 4 x 0.5 OCU |
| Redundancy OFF (dev/test) | 0.5 OCU x 2 | 0.5 OCU x 2 | 2 x 0.5 OCU per workload type |
...
Sizing model addition: `data_size x retention_days / 30 x $/GB-mo` plus PUT cost.
