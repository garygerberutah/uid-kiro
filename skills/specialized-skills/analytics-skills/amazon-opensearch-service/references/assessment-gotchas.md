...
Each entry carries a `**Category:**` tag that determines which lane it surfaces under in the FULL_ASSESSMENT Section 7 split (and which assets it deducts from in [`readiness-rubric.md`](readiness-rubric.md)):

| Category | Meaning | Lane in Section 7 |
|---|---|---|
...
| `CLARIFICATION` | The gotcha is "the customer's claim is wrong / ambiguous"; resolution is pre-work, not a remediation. | Surface as a question, not in either Section 7 lane. |
...
**Fix:** For current OCU minimums, see [`sizing.md` Section OCU model](sizing.md) and [serverless-scaling.html](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/serverless-scaling.html). For tiny non-prod workloads, consider small Managed `t3.medium.search`. NEVER `t2.*` or `t3.small.search` in prod.
...
**Fix:** For current OCU minimums, see [`sizing.md` Section OCU model](sizing.md) and [serverless-scaling.html](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/serverless-scaling.html). If vector is exploratory, run k-NN on existing Managed cluster instead.
...
**Fix:** Add S3 line to sizing model: `data_size x retention_days / 30 x $/GB-mo` plus PUT cost.
...
OR1 stores segments in S3 with local NVMe cache. ~2x r6g indexing throughput, replica=1 sufficient (S3 durable). Loses to r-family on cache-miss aggregations and k-NN graphs (RAM-bound).

**Detect:** k-NN, large-cardinality aggs, or cache-miss-sensitive workloads on OR1
**Fix:** Use OR1 only when `peak_indexing x avg_doc_size > 50 GB/day/node`. Use one replica unless durability model demands more. **Migration to OR1 is irreversible.**
...
**Fix:** For current cluster-manager sizing (heap-to-nodes / shard tier), see [`sizing.md` Section Topology defaults](sizing.md).
