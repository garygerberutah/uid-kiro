...
- **GP2**: Throughput = min(250 MiB/s, max(128 MiB/s, 0.75 x VolumeSize_GiB)). The 250 MiB/s cap is reached at ~334 GiB. Volumes also have `BurstBalance` that depletes under sustained IO.
...
**Calculate effective throughput demand**: `BytesInPerSec x ReplicationFactor`. For RF=3 and 83 MiB/s ingress, total write IO = 250 MiB/s, hitting GP2 ceiling.
