...
- [YES] **CPU-busy formula is correct**: `(CPUUExec + CPUUServ) / (CPUUExec + CPUUServ +
...
- [WARNING] **`ResUsageSpma.MemSize` is NOT physical node RAM** (observed ~126,844 per sample --
  not bytes, ~124 MB-scale). The earlier `MemSize x nodes / 1 GiB` derivation yields ~0 and
