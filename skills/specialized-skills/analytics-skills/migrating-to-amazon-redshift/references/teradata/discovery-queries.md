...
| `DBC.TableSizeV` | permanent space per table and per AMP (skew) | per table x AMP -> SUM |
...
| `DBC.ResUsageSpma` | system resource time series -- often disabled | per node x sample |
