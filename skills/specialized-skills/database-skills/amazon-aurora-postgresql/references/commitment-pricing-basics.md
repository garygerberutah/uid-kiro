...
Effective hourly rate: `(upfront / term_hours) + recurring_hourly` where `term_hours = years x 365 x 24`.

### Size Flexibility

An RI for one instance size in a family covers equivalent normalized units of other sizes. Example:

- 1x `db.r7g.2xlarge` RI can cover 2x `db.r7g.xlarge` OR 4x `db.r7g.large`
...
On I/O-Optimized clusters, compute is charged at 1.30x the standard rate. RI coverage applies to I/O-Optimized compute, but I/O-Optimized draws down RI normalized units 1.3x faster than Aurora Standard. To fully cover an I/O-Optimized cluster, buy ~30% more RIs (e.g., 10 db.r6g.large RIs -> 13 needed -> buy 3 more).
