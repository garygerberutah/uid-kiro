...
```
weighted_cpu = (P95_CPU x 0.95 + Max_CPU x 0.05) / 100
raw_acu = weighted_cpu x vCPU_count x family_ratio
...
- `max_acu = MIN(peak_acu x 1.3, 256)`
- Ensure max >= typical x 1.5 for burst capacity
- If per-instance peak exceeds 256 ACU, workload exceeds serverless capacity on a single instance
- Total cluster peak ACU = per-instance peak x number of instances
...
```
compute = estimated_acu x $0.12/ACU-Hr x 730 hours
storage = storage_gib x $0.10/GiB-month   (Standard; x $0.225/GiB-month for I/O-Optimized)
