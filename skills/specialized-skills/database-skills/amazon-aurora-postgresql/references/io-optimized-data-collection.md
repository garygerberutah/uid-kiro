...
```
monthly_io = (observed_io / observed_hours) x 730
```

Minimum viable window: **7 days**. Below this, Aurora workloads often miss a full weekly cycle (weekdays vs weekends can differ 3-5x), producing misleading extrapolations.
...
```
compute_monthly = sum (instance_price_per_hour x 730) for each instance in cluster
```

The 30% premium multiplies the full compute cost. A cluster with one writer + two readers multiplies the premium by 3x the base instance cost.
