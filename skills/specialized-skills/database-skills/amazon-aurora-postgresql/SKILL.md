...
|---|---|---|
| Default PostgreSQL create, no special networking | [YES] default | -- |
| Quick start / "no VPC setup" / "ready in seconds" | [YES] | -- |
| Customer VPC, subnet group, or specific security group | -- | [YES] required |
| Customer-managed KMS key (CMK) | -- | [YES] required |
| Custom DB cluster parameter group **at creation** | -- | [YES] required |
| Specific engine version pinned by the user | -- | [YES] required (intent to pin = not express) |
