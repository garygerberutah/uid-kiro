...
```
  ...
  |              |    |         |    |           |    |              |
  |  CloudFront  |--->|  HTTP   |--->|  Lambda   |--->|  DynamoDB x9 |
  |  + WAF       |    |  API    |    | (Lambda-  |--->|  S3          |
...
items can span tables but not regions or accounts, 2x the WCU cost of
...
the 100-item cap and 2x cost make it wrong for cascades over a few
items.
...
- **`PAY_PER_REQUEST`** until traffic is forecastable. Break-even with
  **provisioned** capacity is roughly 18% sustained utilization
  ...
  the capacity-mode note in Section 12), but PAY_PER_REQUEST shields you from
  forecasting errors.
...
- SQS visibility timeout >= Lambda timeout x 6 (AWS recommendation for
  Lambda consumers).
...
flat in item size -- measured `dimensions x 4` plus ~35 B regardless of how
...
```
            ...
            |                     |   ARN   |                     |
            |  * DynamoDB x9      |         |  * Web ACL          |
            |  * Lambda x5        |         |  * Managed rules    |
            |  * S3 x2            |         |  * Rate-based rule  |
            |  * CloudFront       |         +---------------------+
            |  * HTTP API         |
            |  * Cognito          |
            |  * AppSync Events   |
...
| Read-after-write needed across an item collection | `ConsistentRead: true` on Query; 2x RCU cost |
