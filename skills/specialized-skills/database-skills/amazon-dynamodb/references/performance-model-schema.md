...
| `warmup_seconds` | `10` | Per-pattern warmup window run at measurement RPS *before* the measurement window. Rows are tagged `phase: "warmup"` and excluded from percentiles and the extrapolation. Reported separately as a cold-start snapshot: patterns where `warmup_p99 > 2 x steady_p99` are flagged `cold_start_elevated: true` in the report. |
...
| `min_rps_per_pattern` | `1` | Lower clamp on post-scale RPS. Ensures every pattern produces at least a handful of samples even when `peak_rps x scale_factor` would round to zero. Raise this if low-traffic patterns are producing percentile columns marked "insufficient samples." |
...
| `cost_guardrail_usd` | `0.50` | **Pre-spend gate.** Before invoking the Lambda, `benchmark_model.py` estimates the actual AWS charge (driven-load CU + seeding writes, priced with the calculator's own per-op CU). If the estimate exceeds this value the run is REFUSED unless `--allow-spend` is passed (the orchestrator forwards it after user consent). Distinct from `abort_on_throttle_rate`, which is a *runtime* guard once the run is underway. Raise it for deliberately larger representative runs. **Vector designs:** the estimate also covers vector search and write bytes, which consume no CU and would otherwise be invisible to the gate -- a `SearchVectors` pattern would estimate at $0. Search is priced there with a deliberately high bound (a gate may be loose, never low), so the gate's figure is *not* a cost estimate; the report's measured figure is. |
...
| Field | Default | Effect |
|---|---|---|
| `read_pattern_key_sampling` | `"uniform"` | How reads pick a target partition. `"uniform"` round-robins over the seeded partitions (even distribution). `"zipf"` (implemented) draws partition ranks with P(rank r) proportional to 1/r^`zipf_s`, concentrating load on a few hot partitions so one partition approaches the per-partition ceiling (Mechanics #3) and hot-partition throttling becomes observable. Auto-enabled by `"representative"` mode. Use `"zipf"` when probing hot-partition behavior; leave `"uniform"` for unit-cost runs. |
| `zipf_s` | `1.1` | Zipf skew exponent (higher = more concentrated). At `1.1` over ~50 partitions the hottest partition absorbs ~25% of traffic and dwarfs the tail ~70x. |
...
- **Sample size matters for percentiles.** `p99` over 60 samples is not a `p99`. If `clamp(peak_rps x scale_factor, min, max) x duration_seconds < 100`, the report flags that pattern as "percentiles are not statistically meaningful" and widens to `p50`/`p95` only. Raise `min_rps_per_pattern` or widen `duration_seconds` to address.
- **Extrapolation assumes linearity.** Extrapolated monthly cost = mean observed CU x declared `peak_rps` x seconds-per-month x unit price. This is valid for per-op unit cost and GSI amplification; it is **not** valid for throttle behavior (throttles scale nonlinearly with partition-key skew -- Mechanics #3). Patterns that hit `max_rps_per_pattern` and had throttles are flagged so the user does not read the extrapolation as a capacity-sustain claim. **`"representative"` mode deliberately surfaces this nonlinearity:** it drives hot-key skew to make throttles appear, and the report quarantines those throttle/latency numbers in a "Load-risk signals" section with an explicit "do not extrapolate" caption. The cost figure stays linear and valid because it uses *declared* peak and scale-invariant per-op CU -- never the elevated driven RPS. Vector bytes extrapolate on the same basis and are similarly scale-invariant per call: metered search bytes measured **exactly** linear in `TopK` within a configuration, and index population is not a linear driver between 10 and 200 vectors in the searched partition because approximate-nearest-neighbour search prunes. One caveat on that last point: an **empty** 1024-dimension index measured roughly **twice** a populated one, so a benchmark run against a barely-seeded vector index can report *higher* per-search bytes than production will -- raise `seed_items_per_table` before treating a vector search figure as representative.
...
- **Vector designs additionally need `botocore`/`boto3 >= 1.43.64`** (AWS CLI v2 `>= 2.36.16`). Below that floor there is no `search_vectors` operation and no `VectorIndexes` parameter at all -- the failure looks like the feature does not exist rather than like a stale SDK. `deploy_model.py` checks the floor and refuses before creating anything. The benchmark Lambda gets a vetted SDK vendored into its bundle from the local install (only for designs that declare a vector index, so every other design keeps its small package), because the Lambda runtime's built-in boto3 is older; the driver also fails loudly if `search_vectors` is missing rather than letting the pattern report zero consumed capacity and read as free.
