...
2. **Disclaimer block.** A blockquote paragraph stating: this is a scaled-down benchmark (duration, scale factor); capacity numbers come from `ReturnConsumedCapacity`; monthly costs extrapolate linearly (observed per-op capacity x declared peak RPS x on-demand public unit price; confirm against the AWS pricing page); the benchmark does **not** prove the design sustains declared peak RPS; what is **not** measured (stream consumers, TTL sweep, autoscaling, cross-region replication, long-tail bursts, non-DDB services).
...
4. **Source-by-source comparison table.** Columns: `Source | Measured Monthly | Calculator | Delta%`. Rows: `Storage (not measured)`, `Read/write requests`.
...
| `Delta` | `(observed - expected) / expected` as percent |
...
| `Extrapolated Monthly` | observed CU x declared `peak_rps` x seconds/month x unit price, **plus** observed vector bytes x declared `peak_rps` x seconds/month x the vector rates. The vector term matters: a `SearchVectors` pattern consumes 0 CU, so without it the pattern extrapolates to exactly $0 and reads as free. See *Vector capacity* below. |

Footnotes below the table:

- `[1] GSI write amplification observed: <r>x (expected <r>x for <projection>)` -- one line per pattern with GSI writes, only if observed and expected differ by >15%.
- `[2] <pattern_id>: <n> throttles during ramp -- see Axiom Findings.` -- one line per pattern that tripped the ramp-throttle flag.
...
| `Elevated?` | `cold_start_elevated` flag (true when warmup p99 > 2 x steady p99) |
...
roughly 10x across configurations, so a predicted figure would be confidently wrong. A live
...
| `Vector $/mo` | bytes/call x peak rps x seconds/month x the per-GB rate |
...
- `Mechanics #7 (projections): GSI <name> with INCLUDE observed <r>x amp -- matches projected-attribute size.`
...
- `<pattern_id>: observed <x> vs expected <y> (Delta <%>). Likely: <RPS off | item size off | conditional fail rate | other>.`
...
The top-level `design_findings.json` also carries **`coverage_incomplete`** (bool): true when the run measured fewer than all declared patterns (or every pattern recorded zero calls). When true the report prints a prominent [WARNING] banner at the very top and `no_significant_findings` is forced false -- a half-finished benchmark never reads as a clean result.
