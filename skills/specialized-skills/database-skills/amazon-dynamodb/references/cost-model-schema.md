...
- `KEYS_ONLY` is **flat** -- `dimensions x 4` plus ~35 B of key overhead, and **independent of
...
- The 1 KB per-request minimum only binds below **256 dimensions** (256 x 4 = 1024 B), so it
...
> `dimensions x 4 + key overhead + the rest of the item` -- if the declared size already counted
...
projection is a ~170x multiplier on bytes returned per result (~89 B for `KEYS_ONLY` against
