...
Flag: [RED] Any results BLOCK the upgrade.
...
|----------|-------|--------|
| `query_cache_type=ON` | [RED] Removed in 8.0 | Performance regression |
| `sql_mode=''` | [YELLOW] 8.0 defaults strict | Apps may break |
| `log_warnings` | [YELLOW] Removed in 8.0 | Replace with `log_error_verbosity` |
| `innodb_strict_mode=OFF` | [YELLOW] 8.0 defaults ON | Preserve in parameter group |
...
Flag: [YELLOW] Use this to record which tables have stale statistics as a pre-upgrade baseline -- it helps you spot post-upgrade plan regressions. A major version upgrade invalidates optimizer statistics, so statistics are recalculated **after** the upgrade -- see the post-upgrade checklist, which scopes `ANALYZE TABLE` to the affected tables in a low-traffic window.
