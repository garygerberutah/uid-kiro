...
- `storage_gb`            = `(merged_row_count x merged_row_size_bytes) / (1024^3)`.
...
- **Original tables:** map 1:1 to CQL. `storage_gb = (row_count x row_size_bytes) / 1024^3` per table.
- **Lookup table** per FK `(source.col -> target.pk)`, named `target_by_source`:
  ...
  - `lookup_storage_gb` = `(target_row_count x lookup_row_size_bytes) / 1024^3`.
- `total_storage_gb`  = sum of original + all lookup storage.
- `reads_per_sec`     = sum of original reads + (FK lookups required per query x reads using them).
...
- `total_storage_gb`  = `merged_storage_gb x (1 + number_of_reverse_indexes)`.
- `reads_per_sec`     = same as Option A (no extra read; correct table picked per query).
- `writes_per_sec`    = `Option A writes_per_sec x (1 + number_of_reverse_indexes)`.
...
- **Storage trade-offs** -- Option C can 2x or 3x storage versus A.
