...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --no-deletion-protection)
      ...
      ;;
    -h|--help)
      echo "Usage: $0 --created-by MODEL_ID [--region REGION] [--tags KEY=VALUE,...] [--no-deletion-protection]"
      echo ""
      echo "Creates an Aurora DSQL cluster in the specified region."
      echo "Deletion protection is enabled by default."
      echo ""
      echo "Options:"
      ...
      echo "  --no-deletion-protection     Disable deletion protection (default: enabled)"
...
# Enable deletion protection by default (use --no-deletion-protection to opt out)
if [[ "$DELETION_PROTECTION" == "true" ]]; then
  CMD+=(--deletion-protection-enabled)
else
  CMD+=(--no-deletion-protection-enabled)
fi
...
echo "Region:             $REGION"
echo ""
echo "Export these environment variables for use with psql-connect.sh and other scripts:"
echo ""
