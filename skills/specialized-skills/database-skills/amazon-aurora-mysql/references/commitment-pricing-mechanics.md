...
`committed_monthly_cost = on_demand_monthly_cost x (1 - discount_pct)`

Then `absolute_savings_per_month = on_demand_monthly - committed_monthly`, and `savings_pct = discount_pct x 100`.

For offline mode, use the on-demand rate from [../serverless-advisory/formulas-and-examples.md Section Provisioned compute pricing](serverless-advisory-formulas-and-examples.md#provisioned-compute-pricing-table-on-demand-us-east-1) or the DSP for Aurora serverless section below. For live mode, the script pulls rates from the AWS Savings Plans + RI Offerings APIs.
