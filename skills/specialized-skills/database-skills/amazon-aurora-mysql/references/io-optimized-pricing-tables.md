...
**Standard total** = `(compute_$hr x 730 x num_instances) + (storage_GiB x $0.10) + (monthly_io_millions x $0.20)`

**I/O-Optimized total** = `(compute_$hr x 1.30 x 730 x num_instances) + (storage_GiB x $0.225) + 0 (no I/O charge)`

**I/O as % of Standard total** = `(monthly_io_millions x $0.20) / Standard_total x 100`
...
| 3-7 days | `short` | **NO.** Weekly patterns (weekday vs weekend I/O ratios often differ 2-3x) can flip the result. Also surface the 30-day cooldown on the Standard -> I/O-Optimized direction as an additional reason to wait. Call the recommendation tentative; minimum wait: reach at least 14 days before acting. |
