...
| Compute multiplier | 1.0x | 1.30x (30% premium) |
...
- Reserved Instances apply to Aurora I/O-Optimized clusters in full, including the 30% premium. Aurora automatically accounts for the price difference: an I/O-Optimized instance consumes 30% more normalized RI units per hour than the same instance on Standard, so it burns down RI capacity ~1.3x faster. There is no portion forced to on-demand rates
