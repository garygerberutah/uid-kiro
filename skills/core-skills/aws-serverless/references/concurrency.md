...
- **Account RPS quota = 10 x account concurrency** (e.g. 1,000 concurrency -> 10,000 RPS, across all functions). This is an account quota, not a per-instance cap. Per-instance throughput = 1 / function duration.
...
| Reserved + Maximum Concurrency (ESM) | Yes | Reserved >= sum(Maximum Concurrency across ESMs) |
...
**RPS gotcha:** a 50ms function at 20,000 RPS needs only 1,000 concurrency, but the RPS limit (10x1,000 = 10,000) throttles it. Request account concurrency = 2,000.
...
6. **Maximum Concurrency > reserved** -- ESM tries 100, function caps at 50. Ensure reserved >= sum(Maximum Concurrency).
