...
Event source mapping (ESM) config parameters, scaling numbers, and filtering gotchas. Assumes you know the basics (SNS pushes asynchronously and is a direct trigger not an ESM; `ReportBatchItemFailures` returns `{batchItemFailures: [{itemIdentifier}]}`; unmatched SQS filter messages are permanently deleted; SQS visibility timeout >= 6x function timeout) -- this file focuses on the exact values and edge cases.
...
Lambda polls stream shards **4x/second**; invokes synchronously, in-order per partition key.
...
- 100 shards x ParallelizationFactor 10 = up to **1,000 concurrent invocations** (order maintained at partition-key level).
...
|--------|---------------|
| SQS | Redrive policy / DLQ (`maxReceiveCount`) always; `ReportBatchItemFailures`; visibility >= 6x timeout; `MaximumConcurrency` to protect downstream |
...
SQS (provisioned):  MaximumPollers x 10
DynamoDB/Kinesis:   number_of_shards x ParallelizationFactor
```
